...
import           Data.Functor       ((<$>))
import           Options.Applicative            (Parser, execParser,switch,
                                                 fullDesc, header, help, helper,
                                                 info, long, metavar,
                                                 progDesc, strOption, (<**>))
import           Data.Int           (Int)
import           Data.List          (concatMap, length, replicate, unlines)
...
copyNew :: FsSource -> FsTarget -> MonthYear -> IO [FsOperation]
copyNew (FsSource source) (FsTarget target) currentMy = do
  sourceCats <- dirsBelowJustName source
...
copyAndMove :: FsSource -> FsTarget -> MonthYear -> MonthYear -> IO [FsOperation]
copyAndMove source target currentMy lastMy = do
  deleteOps <- removeOld target lastMy
  copyOps <- copyNew source target currentMy
...
interpretOperation op = do
  ...
  where
    ...
    interpretOperation' (FsRemoveDir t) = return ()--removeDirectoryRecursive t
...
keepBooks cliVars = do
  ...
  lmy <- lastMonthYear
  copyAndMoveOps <- copyAndMove source target cmy lmy
